...
3) https://drive.google.com/file/d/19SnuK0-BGgRWcDRjy7jbLZvec5MDV1PU датасет для обучения.

### Обновления

* Перевел все "ты" --> "вы". Эксперименты показали куда более лучшие результаты. 
* Оригинальный датасет переведен в двух видах: с блокировкой "вы" и "ты". Все новые модели обучались на сумме двух полученных датасетов.
* Первая попытка реализовать sparse_generation. Доступна модель и метод генерации в настройках игры (при sparse генерации top_p и top_k не работают!)
